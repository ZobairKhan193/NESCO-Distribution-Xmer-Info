...
/* ══════════════════════════════════════════════════
   PROJECT PAGES — PDSSP & NIDMP
══════════════════════════════════════════════════ */
function renderPDSSP() {
  const p = (PROJECTS_DATA && PROJECTS_DATA.pdssp) || null;
  if (!p) {
    document.getElementById('content').innerHTML = `
    <div class="sec-head"><div class="sec-head-left"><h2>PDSSP</h2><p>Project data not loaded.</p></div></div>`;
    return;
  }
  const byZone = {};
  (p.substations || []).forEach(s => { (byZone[s.zone] = byZone[s.zone] || []).push(s); });
  document.getElementById('content').innerHTML = `
  <div class="proj-hero proj-hero-go">
    <div class="proj-tag tag-go">${p.stage} · ${p.funding}</div>
    <h1>${p.name}</h1>
    <p>Strengthening of NESCO's existing 33/11 kV distribution substations.</p>
  </div>

  <div class="kpi-row" style="grid-template-columns:repeat(4,1fr)">
    <div class="kpi-card teal"><div class="kpi-val">${(p.substations||[]).length}</div><div class="kpi-sub">Substations in Scope</div></div>
    <div class="kpi-card indigo"><div class="kpi-val">${Object.keys(byZone).length}</div><div class="kpi-sub">Zones</div></div>
    <div class="kpi-card amber"><div class="kpi-val">${(p.scope||[]).length}</div><div class="kpi-sub">Major Scope Items</div></div>
    <div class="kpi-card green"><div class="kpi-val">NDB</div><div class="kpi-sub">Funding Source</div></div>
  </div>

  <div class="panel">
    <div class="panel-head"><h3>Project Scope</h3></div>
    <div class="panel-body">
      <ul class="scope-list">${(p.scope||[]).map(s => `<li><i class="fas fa-circle-check"></i> ${s}</li>`).join('')}</ul>
    </div>
  </div>

  <div class="panel">
    <div class="panel-head"><h3>Substations under PDSSP</h3>
      <span style="font-size:.78rem;color:var(--text3)">${(p.substations||[]).length} substations across ${Object.keys(byZone).length} zones</span>
    </div>
    <div class="panel-body no-pad"><div class="tbl-wrap">
    <table class="tbl">
      <thead>
        <tr>
          <th>Sl.</th><th>Zone</th><th>Substation</th><th>District</th>
          <th>Power Transformer</th><th>33 kV VCB</th>
          <th>33 kV PCM Panel</th><th>11 kV Switchgear</th>
          <th>New Control Room</th>
        </tr>
      </thead>
      <tbody>
        ${(p.substations||[]).map(s => `
          <tr>
            <td>${s.sl}</td>
            <td><span class="badge badge-blue">${D(s.zone)}</span></td>
            <td><strong>${D(s.name)}</strong></td>
            <td>${D(s.district)}</td>
            <td style="font-size:.82rem">${D(s.power_xfr)}</td>
            <td style="font-size:.82rem">${D(s.vcb)}</td>
            <td style="font-size:.82rem">${D(s.pcm_panel)}</td>
            <td style="font-size:.82rem">${D(s.switchgear)}</td>
            <td style="font-size:.82rem">${D(s.control_room)}</td>
          </tr>`).join('')}
      </tbody>
    </table>
    </div></div>
...
function renderNIDMP() {
  const p = (PROJECTS_DATA && PROJECTS_DATA.nidmp) || null;
  if (!p) {
    document.getElementById('content').innerHTML = `
    <div class="sec-head"><div class="sec-head-left"><h2>NIDMP</h2><p>Project data not loaded.</p></div></div>`;
    return;
  }
  const fmtBdt = v => v == null ? '—' : '৳ ' + (v/1e7).toFixed(2) + ' Cr';
  const fmtUsd = v => v == null ? '—' : '$' + (v/1e6).toFixed(2) + 'M';
  const t = p.totals || {};
  document.getElementById('content').innerHTML = `
  <div class="proj-hero proj-hero-up">
    <div class="proj-tag tag-up">${p.stage} · ${p.funding}</div>
    <h1>${p.name}</h1>
    <p>Comprehensive modernisation: substation upgrades, new GIS &amp; switching SS, smart metering, energy storage, cybersecurity.</p>
  </div>

  <div class="kpi-row" style="grid-template-columns:repeat(4,1fr)">
    <div class="kpi-card indigo"><div class="kpi-val">${(p.ss_upgrade_list||[]).length}</div><div class="kpi-sub">SS Upgrades (AIS)</div></div>
    <div class="kpi-card teal"><div class="kpi-val">${(p.ss_gis_list||[]).length}</div><div class="kpi-sub">SS Upgrades (GIS)</div></div>
    <div class="kpi-card pink"><div class="kpi-val">${fmtBdt(t.ss_upgrade_9_bdt_with_vat + (t.ss_upgrade_gis_4_bdt_with_vat||0) + (t.ss_ais_dinajpur_bdt_with_vat||0))}</div><div class="kpi-sub">SS Upgrade Cost (with VAT)</div></div>
    <div class="kpi-card amber"><div class="kpi-val">ADB</div><div class="kpi-sub">Funding Source</div></div>
  </div>
  ...
  <div class="grid-2col" style="display:grid;gap:18px;grid-template-columns:repeat(auto-fit,minmax(440px,1fr))">
    <div class="panel">
      <div class="panel-head"><h3>9 SS Upgradation (AIS)</h3>
        <span style="font-size:.78rem;color:var(--text3)">${fmtBdt(t.ss_upgrade_9_bdt_with_vat)} · ${fmtUsd(t.ss_upgrade_9_usd_with_vat)}</span>
      </div>
      <div class="panel-body no-pad"><div class="tbl-wrap">
      <table class="tbl"><thead><tr><th>Sl.</th><th>Substation</th><th>Zone</th><th>Cost (BDT)</th></tr></thead>
        <tbody>${(p.ss_upgrade_list||[]).map(s => `<tr>
          <td>${s.sl}</td><td><strong>${s.name}</strong></td><td>${D(s.zone)}</td>
          <td class="num">${fmtBdt(s.bdt)}</td></tr>`).join('')}
        </tbody></table></div></div>
    </div>

    <div class="panel">
      <div class="panel-head"><h3>4 SS Upgradation (GIS)</h3>
        <span style="font-size:.78rem;color:var(--text3)">${fmtBdt(t.ss_upgrade_gis_4_bdt_with_vat)} · ${fmtUsd(t.ss_upgrade_gis_4_usd_with_vat)}</span>
      </div>
      <div class="panel-body no-pad"><div class="tbl-wrap">
      <table class="tbl"><thead><tr><th>Sl.</th><th>Substation</th><th>Zone</th><th>Cost (BDT)</th></tr></thead>
        <tbody>${(p.ss_gis_list||[]).map(s => `<tr>
          <td>${s.sl}</td><td><strong>${s.name}</strong></td><td>${D(s.zone)}</td>
          <td class="num">${fmtBdt(s.bdt)}</td></tr>`).join('')}
        </tbody></table></div></div>
    </div>
  ...
  <div class="panel">
    <div class="panel-head"><h3>Additional Scopes</h3></div>
    <div class="panel-body no-pad"><div class="tbl-wrap">
    <table class="tbl"><thead><tr><th>Item</th><th>Quantity</th><th>Cost (Lac BDT)</th></tr></thead>
      <tbody>${(p.additional_scopes||[]).map(s => `<tr>
        <td><strong>${s.item}</strong></td><td>${D(s.qty)}</td>
        <td class="num">${s.cost_lac != null ? s.cost_lac.toLocaleString() : '—'}</td></tr>`).join('')}
      </tbody></table></div></div>
  </div>
  `;
}
