...
function renderDTOperating() {
  ...
  ${(() => {
    ...
    return `<div class="kpi-row" style="grid-template-columns:repeat(6,1fr)">
      ...
      <div class="kpi-card amber"><div class="kpi-val">${avgTemp}</div><div class="kpi-sub">Avg Oil Temp</div></div>
...
function renderOpRows(rows) {
  if (!rows.length) return `<tr><td colspan="15" class="tbl-empty">No operating records. Admin can add readings using the + Add Reading button.</td></tr>`;
  return rows.map((r,i) => {
    ...
    return `<tr>
      ...
      <td class="num" style="${(_phNum(r.oil_temp)||0) > 80 ? 'color:#dc2626;font-weight:800':''}">${D(r.oil_temp)}</td>
